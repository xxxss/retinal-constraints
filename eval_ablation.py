"""
Ablation: separate the effect of noise augmentation vs adaptive DoG.

  1. baseline:           no DoG, no noise aug
  2. baseline_noiseaug:  no DoG, WITH noise aug  <- isolate noise aug effect
  3. adaptive_dog:       adaptive DoG + noise aug <- full model
...
def train(model, epochs, device, noise_aug=False):
...
def run(mode, epochs, device):
    ...
    if mode == "baseline":
        model = BaselineModel(); na = False
    elif mode == "baseline_noiseaug":
        model = BaselineModel(); na = True
    elif mode == "adaptive_dog":
        model = AdaptiveDoGModel(); na = True
    else:
        raise ValueError(mode)

    t0 = time.time()
    model, tst = train(model, epochs, device, noise_aug=na)
    ...
    if mode == "adaptive_dog":
        sb = next(iter(tst))[0][:8].to(device)
...
if __name__ == "__main__":
    ...
    modes = ["baseline", "baseline_noiseaug", "adaptive_dog"]
    R = {}
    for m in modes:
        R[m] = run(m, 50, device)
    ...
    print(sep)
    print("{:<20} | {:>8} | {:>10} | {:>8}".format("Mode", "Clean", "Corrupted", "Drop"))
    print("-" * 20 + "-+-" + "-" * 8 + "-+-" + "-" * 10 + "-+-" + "-" * 8)
    for m in modes:
        c = R[m]["clean"] * 100
        cr = R[m]["corruptions"]["mean"] * 100
        print("{:<20} | {:>7.1f}% | {:>9.1f}% | {:>7.1f}%".format(m, c, cr, c - cr))
